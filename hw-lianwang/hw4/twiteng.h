...
class TwitEng
{
 public:
   ...
   */
  void addTweet(std::string& username, DateTime& t_time, std::string& text);
 ...
 private:
  ...
  std::map<std::string, User *> users_; // key = username, val = User*
  std::map<std::string, std::set<Tweet *> > hashTags_; // key = hashtag, val = list of Tweets

  User* findUser(std::string name); // helper function that return a user if found, and new if not
