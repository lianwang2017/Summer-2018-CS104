...
###################################################################################################################
# Purpose: Defining several variables that can be used within and outside this class.
# Definitions:
...
###################################################################################################################
# Purpose: Compares the contents of two files by looking for an extact line-by-line match.
# Arguments:
...
def compare_files_with_order( source_file, 
    ...
    with open( source_file, 'r' ) as source, open( comp_file, 'r' ) as comp, open( write_file, 'a' ) as write:
        ...
        comp_lines   = [ x.strip() for x in comp_lines   ]

        if skip_white_space:
            source_lines = remove_values_from_list( source_lines, '' )
            comp_lines   = remove_values_from_list( comp_lines  , '' )
        ...
        return [ number_of_missing_lines, number_of_extra_lines ]
...
def compare_files_without_order( source_file, 
    ...
    with open( source_file, 'r' ) as source, open( comp_file, 'r' ) as comp, open( write_file, 'a' ) as write:
        ...
        # Strips white space from the beginning and end of lines. 
        source_lines = [ x.strip() for x in source_lines ]
        comp_lines   = [ x.strip() for x in comp_lines   ]

        # Removes any empty lines if that option is chosen. 
        if skip_white_space:
            source_lines = remove_values_from_list( source_lines, '' )
            comp_lines   = remove_values_from_list( comp_lines  , '' )

        # Counts the number of FOUND lines and MISSING lines, printing the results
        # to an output file. This method does not use a set, so duplicates are permitable. 
        number_of_missing_lines = 0
        for i in range( starting_line, len( source_lines ) ):
        ...
        # Counts any remaining extra lines and prints them to the results file. 
        number_of_extra_lines = 0
...
def compare_files_with_sets( source_file, 
    ...
    with open( source_file, 'r' ) as source, open ( comp_file, 'r' ) as comp, open( write_file, 'a' ) as write:
        ...
        # Strips the white space from the beginning and end of lines. 
        source_lines = [ x.strip() for x in source_lines ]
        comp_lines   = [ x.strip() for x in comp_lines   ]

        # Removes elements if they are only white space. 
        if skip_white_space:
            source_lines = remove_values_from_list( source_lines, '' )
            comp_lines   = remove_values_from_list( comp_lines  , '' )

        # Removes elements containing the substring ignore_lines_with. 
        source_lines = [ item for item in source_lines if ignore_lines_with not in item ]
        comp_lines   = [ item for item in comp_lines if ignore_lines_with not in item   ]

        # A nested list, where each element is a list separated by the delimiter. 
        source_sets = []
        source_sets.append( [] )
        for i in range( starting_line, len( source_lines ) ):
        ...
        # Removes any empty lists in case extra white space is at the end of the file or more spaces
        # separated the various sets. 
        ...
        return [ number_of_incorrect_sets, number_of_extra_sets ]
...
def run_executable( executable_path,
                    ...
                    use_valgrind = 0 ):
    if use_valgrind:
        os.system( VALGRIND + ' ' + executable_path + executable_name + ' ' + extra_arguments )
    else:
...
###################################################################################################################
def check_file_existence( file_name, directory = '/' ):
    return os.system.path( directory + file_name )
